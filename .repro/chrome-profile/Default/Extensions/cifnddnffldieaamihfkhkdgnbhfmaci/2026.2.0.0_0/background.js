...
var g_InterfaceVersion_V1 = "V1";
var g_InterfaceVersion_V2 = "V2";
var g_InterfaceVersion_V3 = "V3";
var g_SecureMethod_ConvertToPDF = "ConvertToPDF";
var g_NativeMessageSignatureSecret = "FoxitCreatorNativeMessageV3_2026";
...
{
	return Array.from(new Uint8Array(buffer)).map(function (v) {
...
{
	var keys = ["methodname", "action", "URLs", "title", "htmlcontent", "cookies", "langid", "property", "propertyValue", "timestamp", "nonce"];
	return keys.map(function (key) {
		return key + "=" + normalizeFieldValue(method[key]);
	}).join("&");
}
...
function generateNonce()
{
	...
	return Array.from(bytes).map(function (v) {
		return v.toString(16).padStart(2, "0");
	}).join("");
}

function signCanonicalString(canonicalString, callback)
{
	...
	var keyData = encoder.encode(g_NativeMessageSignatureSecret);
	var data = encoder.encode(canonicalString);
	...
	crypto.subtle.importKey(
		...
		callback(toHex(signatureBuffer));
	}).catch(function () {
		callback("");
	});
}

function requiresSecureV3(method)
{
	return method && method.methodname == g_SecureMethod_ConvertToPDF;
}

function prepareSecureV3(method, callback)
{
	method.timestamp = Date.now().toString();
	method.nonce = generateNonce();
	var canonicalString = buildCanonicalString(method);
	signCanonicalString(canonicalString, function (signatureHex) {
		if (!signatureHex)
		{
			callback(false);
			return;
...
function sendNativeMessage(method, callback)
{
	...
	if (g_sNativeMessagingEXEVersion < g_sNativeMessagingEXE82)
	{
		...
		returnValue.error = "SecureProtocolRequired";
		if (callback)
		{
	...
	prepareSecureV3(method, function (securePrepared) {
		if (!securePrepared)
		{
			returnValue.status = 0;
			returnValue.error = "SignatureGenerationFailed";
			if (callback)
			{
				callback(returnValue);
			}
		...
		var methodEncode = Object.assign({}, method);
		...
		if (g_sNativeMessagingEXEVersion >= g_sNativeMessagingEXE82)
		{
			for (var key in methodEncode)
			{
				...
				methodEncode[key] = btoa(encodeURI(normalizeFieldValue(value)));
			}
			methodEncode.interfaceVersion = g_InterfaceVersion_V3;
		}
		else
		{
			methodEncode.interfaceVersion = g_InterfaceVersion_V1;
		}
