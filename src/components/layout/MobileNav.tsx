...
export function MobileNav({ role }: MobileNavProps) {
  ...
  // Verrouillage du scroll du body quand le menu est ouvert
  useEffect(() => {
    if (open) {
  ...
  const drawerContent = (
    ...
    >
      ...
      <div
        className={cn(
          'fixed inset-y-0 left-0 flex w-72 max-w-[85vw] flex-col bg-white shadow-2xl transition-transform duration-300 ease-out dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800',
      ...
      >
        ...
        <div className="flex h-16 items-center justify-between border-b border-slate-200 px-4 dark:border-slate-800">
          <Link href={homeHref} onClick={close} className="flex items-center gap-2.5">
            <div className="grid size-8 place-content-center rounded-lg bg-gradient-to-tr from-primary to-secondary text-white font-bold shadow-xs">
